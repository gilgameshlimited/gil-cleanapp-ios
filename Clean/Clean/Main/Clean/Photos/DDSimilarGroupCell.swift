...
class DDSimilarGroupCell: UITableViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    weak var vc: DDSimilarPhotosController?
    
    var dataArr: [PHAsset] = [] {
        didSet {
            
            self.titleLab.text = "\(self.dataArr.count) \(self.isDuplicate ? DDlocal("DDDuplicate") : DDlocal("DDSimilar"))"
            self.collectionView.reloadData()
            
            DispatchQueue.global().async {
                DDPhotosManager.shared.getImagePHAssetsTotalFileSize(assets: self.dataArr) { sizeStr in
